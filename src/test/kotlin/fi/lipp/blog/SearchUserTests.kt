...
class SearchUserTests : UnitTestBase() {
    ...
    @Test
    fun `search does not return system user`() {
        transaction {
            // Get or create system user
            val systemUserId = userService.getOrCreateSystemUser()

            // Sign up regular test users
            val (userId1, userId2) = signUsersUp()

            // Search with a term that would match the system user
            val results = userService.search("system")

            // Verify system user is not in results
            for (user in results) {
                assertNotEquals("system", user.login)
                assertNotEquals("system", user.nickname)
            }
